...
const DeviceCard = ({ item }) => {
  ...
  const total = item.discos ? item.discos.reduce((sum, disk) => sum + parseGB(disk.total), 0) : 0;
  const used = item.discos ? item.discos.reduce((sum, disk) => sum + parseGB(disk.usado), 0) : 0;
